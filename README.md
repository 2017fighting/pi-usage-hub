...
2. **`/usage` answers "what can I use right now?"** available providers first, exhausted ones ordered by **how soon their quota resets**, so the provider that comes back first is at the top.
...
- **CodeBuddy** sells credit packages. The total is the sum of every package; each package carries its own cycle end time. Authentication uses the CodeBuddy OAuth JWT Pi already stores — no browser cookie is needed.
- **DeepSeek** is a prepaid balance with no reset. It reports as `exhausted` when the balance reaches zero.
...
- **Kimi** reports a 5h window and a monthly pool; the monthly pool can have room while 5h is exhausted, so the provider is still "available".
- A provider is **available** when at least one of its windows still has room. Fetch failures are reported as `unknown`, never as zero.
